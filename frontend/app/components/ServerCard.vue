<template>
    <div
        :class="[
            'bg-card border rounded-xl p-5 flex flex-col gap-4 transition-[border-color,box-shadow] duration-300',
            cardClass,
        ]"
    >
        <div class="flex items-start justify-between gap-2">
            <div class="flex flex-col gap-0.5">
                <span class="text-base font-bold text-fore">{{
                    server.name
                }}</span>
        ...
        <template v-if="server.metric">
            ...
            <div class="grid grid-cols-4 gap-2 border-t border-border pt-3">
                <div class="flex flex-col gap-0.5">
                    <span
                        class="text-[0.65rem] text-muted uppercase tracking-wide"
                        >Response</span
                    >
                    <span
                        ...
                        :style="{
                            color:
                                server.metric.response_time >= 1000
                                    ? 'var(--color-warning)'
                                    : 'var(--color-success)',
                        }"
                ...
                <div class="flex flex-col gap-0.5">
                    <span
                        ...
                        >Req/s</span
                    >
                    <span class="text-sm font-semibold text-fore">{{
                        server.metric.request_rate
                    }}</span>
                </div>
                <div class="flex flex-col gap-0.5">
                    <span
                    ...
                    >
                    <span class="text-sm font-semibold text-fore">{{
                        fmtBytes(server.metric.network_in)
                    }}</span>
                </div>
                <div class="flex flex-col gap-0.5">
                    <span
                    ...
                    >
                    <span class="text-sm font-semibold text-fore">{{
                        fmtBytes(server.metric.network_out)
                    }}</span>
                </div>
            </div>
            ...
            <div
                v-if="server.updatedAt"
                class="text-[0.65rem] text-muted text-right -mt-1"
            >
...
<script setup lang="ts">
    ...
    const cardClass = computed(() => {
        switch (props.server.status) {
            ...
            case 'critical':
                return 'border-danger/50 shadow-[0_0_12px_rgba(239,68,68,0.1)]'
            case 'offline':
