<template>
    <div class="min-h-screen bg-surface flex items-center justify-center p-4">
        <div
            class="bg-card border border-border rounded-2xl p-10 w-full max-w-sm"
        >
            <div
                class="flex items-center gap-2.5 text-xl font-bold text-fore mb-1"
            >
                <div
                    class="w-3 h-3 rounded-full bg-accent shadow-[0_0_10px_var(--color-accent)]"
                />
            ...
            <form class="flex flex-col gap-4" @submit.prevent="submit">
                ...
                <p
                    v-if="errorMsg"
                    class="text-xs text-danger bg-danger/10 rounded-md px-3 py-2"
                >
                    {{ errorMsg }}
                </p>
...
<script setup lang="ts">
    ...
    async function submit() {
        ...
        try {
            ...
            errorMsg.value =
                e?.data?.message ?? 'Login failed. Check your credentials.'
        } finally {
            loading.value = false
        }
    }
