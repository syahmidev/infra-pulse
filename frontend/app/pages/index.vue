    ...
    <div>
        <div class="flex items-start justify-between mb-6 flex-wrap gap-4">
            ...
            <div class="flex gap-3">
                <div
                    class="flex flex-col items-center bg-card border border-border rounded-xl px-4 py-2 min-w-[64px]"
                >
        ...
        <div v-if="loading" class="text-center py-16 text-muted text-sm">
            Loading servers…
        </div>
        <div v-else-if="error" class="text-center py-16 text-danger text-sm">
            {{ error }}
        </div>
...
<script setup lang="ts">
    ...
    const activeServers = computed(
        () => servers.value.filter((s) => s.is_active).length,
    )
